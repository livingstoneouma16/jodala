...
from app.auth import login_required, role_required, get_current_user, hash_password
from app.serializers import (loan_product_public, user_public, notification_public,
                              audit_log_public, member_full_name, client_full_name)
from app.utils import paginate, log_audit, adjust_main_account_balance, adjust_account_balance, notify
...
def get_notification_settings():
    from app.mailer import get_mail_config
    cfg = get_mail_config()
    return jsonify({
...
@role_required('admin')
def get_email_log():
    from app.mailer import get_recent_email_log
...
def send_test_notification_email():
    from app.mailer import send_email, is_configured
...
def get_mpesa_settings():
    from app.mpesa import get_b2c_config, is_b2c_configured, _setting
    cfg = get_b2c_config()
    return jsonify({
...
def update_mpesa_settings():
    ...
    if data.get('mpesa_b2c_certificate'):
        ...
        from app.mpesa import encrypt_security_credential, MpesaError
        try:
...
@role_required('admin')
def get_mpesa_log():
    from app.mpesa import get_recent_mpesa_log
...
def send_test_mpesa_push():
    from app.mpesa import initiate_stk_push, MpesaError, normalize_phone
    from app.routes.mpesa import _callback_url
    data = request.get_json() or {}
...
def send_test_mpesa_b2c():
    from app.mpesa import initiate_b2c_payment, MpesaError, normalize_phone
    from app.routes.mpesa import _b2c_result_url, _b2c_timeout_url
    data = request.get_json() or {}
    phone = data.get('phone')
    if not phone:
        return jsonify({'error': 'Phone number is required'}), 400
...
def update_user(user_id):
    ...
    if data.get('password'):
        ...
        execute(
            "UPDATE users SET full_name=?, email=?, phone=?, role=?, is_active=?, password_hash=?, updated_at=? WHERE id=?",
            (full_name, email, phone, role, is_active, password_hash, utcnow(), user_id)
        )
    else:
        execute(
