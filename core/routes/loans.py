...
@loans_bp.route('/api/<int:loan_id>/push-schedule', methods=['POST'])
@login_required
@role_required('admin', 'loan_officer')
def push_schedule(loan_id):
    """Shift every pending/partial installment's due date by N days (can be
    negative to pull dates in) without touching principal, interest, or any
    other amount -- unlike restructure/extend, which re-amortize the loan.
    Paid installments are left untouched. expected_end_date is kept in sync
    with the new final due date, and the shift is written to audit_logs."""
    loan = get_db().execute("SELECT * FROM loans WHERE id = %s", (loan_id,)).fetchone()
    if not loan:
        return jsonify({'error': 'Loan not found'}), 404
    if loan['status'] != 'active':
        return jsonify({'error': 'Can only push the schedule of an active loan'}), 400

    data = request.get_json() or {}
    reason = (data.get('reason') or '').strip()
    if not reason:
        return jsonify({'error': 'A reason is required to push the schedule'}), 400
    ...
    if days == 0:
        return jsonify({'error': 'days must be non-zero'}), 400
    ...
    if not rows:
        return jsonify({'error': 'No pending or partially-paid installments to push'}), 400

    old_dates = {r['id']: r['due_date'] for r in rows}

    new_last_due_date = None
    for r in rows:
        shifted = (date.fromisoformat(r['due_date']) + timedelta(days=days)).isoformat()
        execute("UPDATE loan_schedules SET due_date = %s WHERE id = %s", (shifted, r['id']))
        new_last_due_date = shifted

    if new_last_due_date and new_last_due_date != loan['expected_end_date']:
        execute(
            "UPDATE loans SET expected_end_date = %s, updated_at = %s WHERE id = %s",
            (new_last_due_date, utcnow(), loan_id)
        )

    log_audit(
        'LOAN_SCHEDULE_PUSHED', 'loan', loan_id,
        old_values={'due_dates': old_dates},
        new_values={'days_shifted': days, 'reason': reason}
    )
    ...
    return jsonify({
        'message': f'Schedule pushed by {days} day(s)',
