...
@loans_bp.route('/api/<int:loan_id>/schedule/<int:schedule_id>/due-date', methods=['PATCH'])
@login_required
@role_required('admin', 'loan_officer')
def update_installment_due_date(loan_id, schedule_id):
    """Correct a single installment's due date -- e.g. it was entered wrong,
    or a borrower needs one installment pushed out without a full
    restructure/extend of the rest of the loan. Only pending/partial
    installments can be moved; paid ones are a historical record and stay
    put. Every change is written to audit_logs (old -> new date, and the
    reason if one was given) so this can't be used to quietly erase
    overdue history."""
    loan = get_db().execute("SELECT * FROM loans WHERE id = %s", (loan_id,)).fetchone()
    if not loan:
        return jsonify({'error': 'Loan not found'}), 404
    ...
    schedule_row = get_db().execute(
        "SELECT * FROM loan_schedules WHERE id = %s AND loan_id = %s", (schedule_id, loan_id)
    ).fetchone()
    if not schedule_row:
        return jsonify({'error': 'Installment not found'}), 404

    if schedule_row['status'] not in ('pending', 'partial'):
        return jsonify({'error': 'Only pending or partially-paid installments can have their due date changed'}), 400

    data = request.get_json() or {}
    new_due_date = (data.get('due_date') or '').strip()
    reason = (data.get('reason') or '').strip()
    ...
    old_due_date = schedule_row['due_date']
    if new_due_date == old_due_date:
        return jsonify({'error': 'New due date is the same as the current due date'}), 400

    execute(
        "UPDATE loan_schedules SET due_date = %s WHERE id = %s",
        (new_due_date, schedule_id)
    )

    # If this pushes the last installment out, keep the loan's
    # expected_end_date consistent with the actual schedule rather than
    # leaving it pointing at a date that's no longer the true final due date.
    last_due_row = get_db().execute(
        "SELECT MAX(due_date) AS d FROM loan_schedules WHERE loan_id = %s", (loan_id,)
    ).fetchone()
    if last_due_row and last_due_row['d'] and last_due_row['d'] != loan['expected_end_date']:
        execute(
            "UPDATE loans SET expected_end_date = %s, updated_at = %s WHERE id = %s",
            (last_due_row['d'], utcnow(), loan_id)
        )

    log_audit(
        'LOAN_INSTALLMENT_DUE_DATE_CHANGED', 'loan', loan_id,
        old_values={'schedule_id': schedule_id, 'installment_number': schedule_row['installment_number'],
                    'due_date': old_due_date},
        new_values={'schedule_id': schedule_id, 'installment_number': schedule_row['installment_number'],
                    'due_date': new_due_date, 'reason': reason or None}
    )

    updated_schedule = get_db().execute(
        "SELECT * FROM loan_schedules WHERE loan_id = %s ORDER BY installment_number", (loan_id,)
    ).fetchall()

    return jsonify({
        'message': 'Due date updated',
        'schedule': [loan_schedule_public(s) for s in updated_schedule]
    }), 200
