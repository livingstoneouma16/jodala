...
@loans_bp.route('/api/<int:loan_id>/reschedule', methods=['POST'])
@login_required
@role_required('admin', 'loan_officer')
def reschedule_loan(loan_id):
    """Move every pending/partial installment's due date by N days without
    recalculating anything -- principal, interest, and totals for each
    installment stay exactly as they were. Unlike Restructure, which
    re-amortizes the remaining principal over a new term/rate (and so
    produces a new interest figure), this only shifts dates."""
    loan = get_db().execute("SELECT * FROM loans WHERE id = %s", (loan_id,)).fetchone()
    if not loan:
        return jsonify({'error': 'Loan not found'}), 404
    if loan['status'] != 'active':
        return jsonify({'error': 'Can only reschedule an active loan'}), 400

    data = request.get_json() or {}
    reason = (data.get('reason') or '').strip()
    if not reason:
        return jsonify({'error': 'A reason is required to reschedule a loan'}), 400
    ...
    if not rows:
        return jsonify({'error': 'No pending or partially-paid installments to reschedule'}), 400
    ...
    return jsonify({
        'message': f'Schedule rescheduled by {days} day(s) -- no interest added',
