"""
Email sending via Gmail SMTP with an app password.

IMPORTANT: Gmail SMTP connects on port 465 (SSL) or 587 (STARTTLS). Render,
Fly.io, and most free/cheap PaaS hosts block outbound traffic on SMTP ports
at the network level (to stop the platform being used for spam), so every
send will fail there with something like "[Errno 101] Network is
unreachable" no matter how correct the credentials are. This approach is
only appropriate for hosts that allow outbound SMTP (a VPS, your own
server, etc.) or local/dev use. If this app ends up on Render/Fly/similar,
switch to an HTTP-based sender (e.g. the Gmail API, or a transactional
...
Setup:
  ...
  2. Create an App Password at https://myaccount.google.com/apppasswords
     ...
     use that, not the account's normal login password.
  3. Set the Gmail address + app password either in Settings >
     Notifications, or via env vars (see below).

Credentials can come from either source, checked in this order:
  1. The `company_settings` DB table (keys: gmail_address,
     gmail_app_password, gmail_sender_name) -- set from the Settings >
...
def _try_send(cfg, to_email, subject, body_text, body_html):
    ...
    try:
        ...
        return False, (
            'Gmail rejected the credentials -- check the address and App Password under '
            f'Settings > Notifications (make sure 2-Step Verification is on and this is an '
            f'App Password, not the normal login password). ({e})'
        )
    except (smtplib.SMTPException, OSError) as e:
        return False, (
            f'Could not send via Gmail SMTP: {e}. If this app is hosted on Render, Fly.io, or '
            'a similar PaaS, this is very likely because outbound SMTP ports are blocked at the '
            'network level there -- SMTP-based email cannot work on those hosts regardless of '
            'credentials; an HTTP-based sender (e.g. the Gmail API) would be needed instead.'
        )


def send_email(to_email, subject, body_text, body_html=None):
    """
    Send a single email via Gmail SMTP. Returns
    ...
    if not is_configured():
        logger.warning("Skipped email '%s' to %s: Gmail SMTP credentials not configured", subject, to_email)
