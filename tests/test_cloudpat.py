"""
CloudPat gateway tests -- mirrors test_mpesa.py's structure since CloudPat
is a switchable drop-in alternative to M-Pesa Daraja for STK Push (see
core/routes/mpesa.py:get_active_gateway()).

Covers:
1. `POST /mpesa/api/stkpush` routes to CloudPat instead of Daraja once
   `payment_gateway` is set to 'cloudpat', and the stored mpesa_transactions
   row is tagged with gateway='cloudpat'. The actual CloudPat HTTP call
   (core.cloudpat.initiate_stk_push) is mocked, same as the Daraja test does
   for core.mpesa.initiate_stk_push.
2. `POST /mpesa/cloudpat/callback` -- applies a confirmed payment via the
...
Run with: pytest tests/test_cloudpat.py -v
"""
...
def _cloudpat_success_body(checkout_request_id, amount=1000, receipt='CP-RECEIPT-1'):
    return {
...
def _cloudpat_failure_body(checkout_request_id, result_code=1, desc='Request cancelled by user'):
    return {
        'checkout_request_id': checkout_request_id,
...
class TestGatewaySelection:
    def test_default_gateway_is_mpesa(self, client, admin_token, approved_loan):
        ...
        # go to Daraja, not CloudPat, so existing installs are unaffected.
        with patch('core.routes.mpesa.initiate_stk_push') as mock_mpesa, \
             patch('core.routes.mpesa.cloudpat_initiate_stk_push') as mock_cloudpat:
        ...
        mock_cloudpat.assert_not_called()

    def test_stkpush_routes_to_cloudpat_when_active(self, client, db_conn, admin_token, approved_loan):
        _set_active_gateway(db_conn, 'cloudpat')
        with patch('core.routes.mpesa.cloudpat_initiate_stk_push') as mock_cloudpat, \
             patch('core.routes.mpesa.initiate_stk_push') as mock_mpesa:
            mock_cloudpat.return_value = {
...
class TestCloudPatCallback:
    def _seed_pending_transaction(self, db_conn, loan_id, checkout_request_id='cp_CO_abc', amount=1000):
        ...
        db_conn.execute(
            """INSERT INTO mpesa_transactions (checkout_request_id, merchant_request_id, purpose,
               ...
               VALUES (%s, %s, 'loan_repayment', %s, %s, %s, 'pending', %s, 'cloudpat', now()::text, now()::text)""",
    ...
    def test_successful_callback_applies_repayment_to_loan(self, client, db_conn, approved_loan):
        ...
        resp = client.post('/mpesa/cloudpat/callback', json=_cloudpat_success_body('cp_CO_ok', amount=1000))
    ...
    def test_failed_callback_does_not_touch_loan_balance(self, client, db_conn, approved_loan):
        ...
        resp = client.post('/mpesa/cloudpat/callback', json=_cloudpat_failure_body('cp_CO_fail'))
        assert resp.status_code == 200  # always 200 so CloudPat doesn't retry forever
    ...
    def test_unknown_checkout_request_id_ignored_gracefully(self, client):
        resp = client.post('/mpesa/cloudpat/callback', json=_cloudpat_success_body('cp_CO_never_seen'))
        assert resp.status_code == 200
        assert resp.get_json()['result_code'] == 0

    def test_malformed_callback_body_does_not_500(self, client):
        resp = client.post('/mpesa/cloudpat/callback', json={'nonsense': True})
        assert resp.status_code == 200
    ...
    def test_replayed_callback_does_not_double_apply(self, client, db_conn, approved_loan):
        ...
        first = client.post('/mpesa/cloudpat/callback', json=_cloudpat_success_body('cp_CO_replay', amount=1000))
        assert first.status_code == 200
        # Same reasoning as the Daraja test -- a callback delivered twice
        # must be a no-op the second time, not a double deduction.
        second = client.post('/mpesa/cloudpat/callback', json=_cloudpat_success_body('cp_CO_replay', amount=1000))
    ...
    def test_savings_deposit_purpose_applies_to_savings_account(self, client, db_conn, admin_token, member):
        ...
        # helper still branches correctly on purpose for the CloudPat path,
        ...
        db_conn.execute(
            """INSERT INTO mpesa_transactions (checkout_request_id, merchant_request_id, purpose,
                   target_id, phone, amount, status, initiated_by, gateway, created_at, updated_at)
               VALUES (%s, %s, 'savings_deposit', %s, %s, %s, 'pending', %s, 'cloudpat', now()::text, now()::text)""",
            ('cp_CO_savings', 'cp-mr-2', account['id'], '254712345678', 500, admin_id)
        )
        db_conn.commit()

        resp = client.post('/mpesa/cloudpat/callback', json=_cloudpat_success_body('cp_CO_savings', amount=500))
        assert resp.status_code == 200
        txn = db_conn.execute(
